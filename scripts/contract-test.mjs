...
assert(sharedGuidance.includes("start_quiz only creates a draft") && sharedGuidance.includes("As soon as the first renderable question is ready, call add_first_question exactly once"), "shared builder guidance must prefer immediate add_first_question launch");
assert(sharedGuidance.includes("do not wait until all questions are authored") && sharedGuidance.includes("add_question may be used for the first question as a compatibility launch path") && sharedGuidance.includes("Continue add_question/repair_question exactly once per later question"), "shared builder guidance must add one question at a time with stale-session compatibility");
assert(sharedGuidance.includes("Do not preview, brainstorm, list, or spoil quiz questions in chat text") && sharedGuidance.includes("Use <u>...</u> sparingly"), "shared builder guidance must prevent chat spoilers and allow underline emphasis");
...
const startQuizToolBlock = server.slice(server.indexOf('name: "start_quiz"'), server.indexOf('inputSchema: START_QUIZ_INPUT_SCHEMA', server.indexOf('name: "start_quiz"')));
const addFirstToolBlock = server.slice(server.indexOf('name: "add_first_question"'), server.indexOf('inputSchema: ADD_QUESTION_INPUT_SCHEMA', server.indexOf('name: "add_first_question"')));
const addQuestionToolBlock = server.slice(server.indexOf('name: "add_question"'), server.indexOf('inputSchema: ADD_QUESTION_INPUT_SCHEMA', server.indexOf('name: "add_question"')));
assert(!startQuizToolBlock.includes("fill_blank") && !startQuizToolBlock.includes("numeric"), "start_quiz description must not match question-type discovery queries");
...
assert(!server.includes('name: "add_question",\n    description: "Add exactly one later question') || !server.includes('"openai/outputTemplate": "ui://widget/betterquizzes-v72-feedback-polish.html", "openai/widgetAccessible": true, "openai/toolInvocation/invoking": "Adding question..."'), "storage-only add_question must not advertise an output template");
assert(server.includes('"openai/toolInvocation/invoking": "Checking quiz..."') && !server.includes('"openai/outputTemplate": "ui://widget/betterquizzes-v72-feedback-polish.html", "openai/widgetAccessible": true, "openai/toolInvocation/invoking": "Checking quiz..."'), "recovery-only open_quiz must not advertise an output template");
...
assert(server.includes("betterquizzes-v72-feedback-polish.html"), "widget URI must cache-bust the feedback polish fix");
...
assert(server.includes("betterquizzes-v65-final-hardening.html"), "v65 widget URI must remain as a compatibility alias");
...
assert(server.includes("Duplicate empty draft start detected") && server.includes("v23FindDuplicateEmptyStartDraft"), "start_quiz must guard exact repeated empty draft creation");
assert(server.includes("normalized object choice answer to zero-based index"), "choiceId answer objects must normalize to renderer answer indexes");
