...
function readPngRgba(file) {
  ...
  assert(isPng(bytes), file + " is not PNG");
...
for (const file of [
  ...
  // Submission requirement: square icon, no baked rounded corners/circular mask.
...
if (failures.length) {
  console.error("V31 square icon guards failed:");
  for (const failure of failures) console.error(" - " + failure);
  process.exit(1);
}

console.log("V31 square icon guards passed.");
