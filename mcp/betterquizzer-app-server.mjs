...
10. For fill_blank, short_answer, and long_response questions, you may set responseLimit.maxChars when a limit is useful. Omit responseLimit or set maxChars:null for unlimited. Unlimited fields show no character counter.
...
function buildCompletionSummary(quiz, answers, displayPolicy = normalizeDisplayPolicy(quiz.displayPolicy), activityPolicy = normalizeActivityPolicy(quiz.activityPolicy)) {
  ...
  for (const question of required) {
    ...
    if (displayPolicy.requireConfidence && answerHasResponse(answer) && !isValidConfidence(answer.confidence)) missingRequiredConfidenceIds.push(question.id);
  }
  return {
