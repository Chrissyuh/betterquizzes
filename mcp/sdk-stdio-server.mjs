...
3. A valid QuizSpec needs schema="betterquizzer.quiz", version=2, title, mode, displayPolicy, gradingPolicy, activityPolicy, and questions. Prefer gradingPolicy.preferredGrader="llm" and omit answer keys unless the user asks for scored keys.
4. Generate valid question data. multiple_choice and multi_select require a non-empty choices array. text_select requires segments:[{id,text,selectable?}], optional selectionPolicy, and answer:string[]; use text_select only for a contextual passage with at least two sentences or 120 characters and at least three plausible selectable segments. Do not use choices for text_select and do not make one obvious highlighted phrase. matching requires canonical left:[{id,text}], right:[{id,text}]; answer:[{leftId,rightId}] is optional. Do not author matching as pairs unless repairing legacy input. ordering requires an items array and orderingBehavior.direction exactly "top_to_bottom"; answer item ids are optional; never use first_to_last, chronological, or other conceptual values as direction. Put conceptual meaning in orderingBehavior.topLabel and orderingBehavior.bottomLabel. If unsure, use fill_blank, short_answer, or long_response instead of inventing an invalid structure.
...
server.registerTool(
  "create_quiz",
  {
    ...
    description: "Compatibility opener for a complete BetterQuizzes QuizSpec v2 packet. Use builder start_quiz/add_question/finalize_quiz instead when builder tools are available. Text questions may include responseLimit.maxChars; omit it or set maxChars:null for unlimited.",
    inputSchema: {
