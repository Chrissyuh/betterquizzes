export const V2_BUILDER_INSTRUCTION_LINES = [
  ...
  "For normal assistant-authored quizzes, build quietly. Do not send chat progress/check-in messages while authoring. Start with start_quiz and expectedQuestionCount, then call add_question exactly once for each question. The first accepted add_question launches the widget immediately; continue add_question/repair_question silently while the launched widget refreshes from the stored draft. The normal path has no separate validation step. Do not call open_quiz for the same quiz unless the first add_question launch failed. Do not send question batches in start_quiz.",
