export const V2_BUILDER_INSTRUCTION_LINES = [
  ...
  "For normal assistant-authored quizzes, build quietly. Do not send chat progress/check-in messages while authoring. Start with start_quiz and expectedQuestionCount, call add_question exactly once for question 1, then call open_quiz once to show the widget. After that, continue add_question/repair_question once per later question; do not call open_quiz again. The already-open widget polls the stored draft and refreshes as accepted questions arrive. The normal path has no separate validation step. Do not send question batches in start_quiz.",
