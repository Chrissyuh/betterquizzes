export const V2_BUILDER_INSTRUCTION_LINES = [
  ...
  "For normal assistant-authored quizzes, build quietly. Do not send chat progress/check-in messages while authoring. Start with start_quiz and expectedQuestionCount, then alternate add_question and open_quiz: call add_question exactly once for question 1, call open_quiz to show it, then after every later accepted add_question call open_quiz again to refresh the visible widget. The normal path has no separate validation step. Do not send question batches in start_quiz.",
