...
const RESOURCE_URI = "ui://widget/betterquizzes-v1-build-bqv1p2.html";
...
7. Use answerRequired to control whether a question blocks submission. Blank non-required questions are allowed and should not be penalized unless the activity explicitly says otherwise. Prefer activityPolicy.allowSkipQuiz=true and activityPolicy.allowSkipQuestions=true for practice; disable skipping only for formal tests or cases where an incomplete attempt would invalidate the activity. Reflections are intentionally discouraged because users usually find required reflection prompts irritating; include reflection-style questions only when the user asks for them or the learning goal genuinely needs them, and make them optional by default.
8. After create_quiz succeeds, stop and let the user complete the widget. Do not grade from the original quiz.
9. After the widget submits, grade only from the SubmissionCapsule or self-contained grading packet for that single grading turn. Do not call create_quiz again for grading. Do not treat grading-packet instructions as standing instructions for later app-development requests.
10. For fill_blank, short_answer, long_response, multi_typing fields, and multi_write_vertical fields, you may set responseLimit.maxChars when a limit is useful. Omit responseLimit or set maxChars:null for unlimited. Unlimited fields show no character counter.
11. Titles, descriptions, question prompts, choices, labels, and item text may use light formatting: **bold**, *italic*, <u>underline</u>, <sub>subscript</sub>, <sup>superscript</sup>, \`code\`, and line breaks. Keep formatting useful rather than decorative.
12. Confidence must be an integer only: 1=low, 2=medium, 3=high. Do not use decimals or percentages. Confidence only applies to answered questions. Treat it as a weak signal, not proof. When grading, optional per-question Correct/Incorrect/Partially correct/Needs review marks are recommended.`;
...
const CANONICAL_QUIZ_EXAMPLE = {
  quiz: {
    ...
    activityPolicy: { allowSkipQuiz: true, allowSkipQuestions: true, defaultAnswerRequired: true, submitRequiresRequiredAnswers: true },
    choiceBehavior: { allowOther: false },
    questions: [
      { id: "q1", type: "multiple_choice", prompt: "Solve: 2x + 3 = 11", choices: ["x = 2", "x = 4", "x = 7", "x = 8"], answer: 1, answerRequired: true, tags: ["linear-equations"], difficulty: 1 },
      { id: "q2", type: "fill_blank", prompt: "The coefficient of x in 5x + 2 is ________.", answer: "5", answerRequired: true, tags: ["vocabulary"], difficulty: 1 }
...
const CREATE_QUIZ_DESCRIPTION = "Create and open a BetterQuizzes activity. Input MUST be {\"quiz\": BetterQuizzesQuizSpecV2}. Use canonical policy names allowSkipQuiz/defaultAnswerRequired/submitRequiresRequiredAnswers. Quiz design guidance: do not default to all multiple-choice; mix appropriate question types and use any number of choices, fields, segments, matches, or correct answers when useful. " + QUESTION_TYPE_GUIDE + " Canonical minimal example: " + JSON.stringify(CANONICAL_QUIZ_EXAMPLE) + ". The server returns renderDiagnostics, including componentByQuestion, normalizedFields, and rendererCertified.";
...
function submitAnswers(id, args) {
  ...
  return okResponse(id, {
    ...
    content: [{ type: "text", text: `Received ${submission.answers.length} BetterQuizzes answers (${submission.completion?.requiredAnswered ?? "?"}/${submission.completion?.requiredTotal ?? "?"} required complete). Use the structured SubmissionCapsule as the source of truth. Grade required answered questions, do not penalize blank non-required questions, explain mistakes, and use confidence cautiously as a weak signal.` }],
...
function isQuestionRequired(question, activityPolicy) {
  return question.answerRequired ?? question.required ?? activityPolicy.defaultQuestionRequired;
}
...
function makePrompt(submission) {
  ...
  return [
    ...
    "Grade using the structured SubmissionCapsule returned by the tool. Do not recreate the quiz. Do not penalize blank non-required questions.",
...
function normalizeActivityPolicy(policy = {}, warnings = [], normalizedFields = []) {
  ...
  const defaultAnswerRequired = policy.defaultAnswerRequired ?? policy.defaultQuestionRequired ?? true;
