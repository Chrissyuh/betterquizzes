...
const V23_BUILDER_TOOL_DEFS = [
  {
    name: "start_quiz",
    description: "Start a BetterQuizzes draft. For normal assistant-authored quizzes, set expectedQuestionCount, add or repair one question at a time until the expected count is reached, then call finalize_quiz once to launch the complete stored draft. " + V2_BUILDER_INSTRUCTIONS,
  ...
  {
    name: "add_question",
    description: "Add one validated question to an incremental BetterQuizzes draft. Required input shape: { draftId, question }. This stores one question and does not open the widget. Continue add_question/repair_question until the expected count is reached, then call finalize_quiz once. Matching canonical shape: {type:'matching', left:[{id,text}], right:[{id,text}]}; optional grading keys use answer:[{leftId,rightId}]. Legacy pairs/matches/items are normalized.",
  ...
  {
    name: "finalize_quiz",
    description: "Finalize an incremental BetterQuizzes draft and launch the complete activity in the widget. For assistant-authored quizzes, call this once after add_question/repair_question has reached the expected question count.",
