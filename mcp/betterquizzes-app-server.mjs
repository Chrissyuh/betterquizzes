...
const V23_BUILDER_TOOL_DEFS = [
  ...
  {
    name: "finalize_quiz",
    description: "For assistant-authored quizzes, this is the launch step: validate the draft through the render contract, store the canonical quizId, and open the BetterQuizzes activity. Do not call another quiz creation or launch tool for the same user request unless explicitly asked.",
...
const QUESTION_TYPE_GUIDE = "Question answer shapes: multiple_choice answer=zero-based index; multi_select answer=zero-based indexes; true_false answer=boolean; numeric answer=number plus optional tolerance; fill_blank/short_answer answer=string or string[] plus optional acceptableAnswers and optional responseLimit.maxChars; ordering answer=ordered item ids in visual top-to-bottom order with orderingBehavior labels when direction matters; matching answer=[{leftId,rightId}]. Light formatting includes LaTeX math using \\(...\\) or \\[...\\]. Do not use dollar-sign math delimiters. Keep compact labels short for mobile.";
