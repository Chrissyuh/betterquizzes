...
const V23_BUILDER_TOOL_DEFS = [
  {
    name: "start_quiz",
    description: "Start a BetterQuizzes draft. For normal assistant-authored quizzes, set expectedQuestionCount, call add_question exactly once for the first question, then immediately call open_quiz once before adding question 2. Later add_question/repair_question calls update the stored draft. " + V2_BUILDER_INSTRUCTIONS,
  ...
  {
    name: "add_question",
    description: "Add one validated question to an incremental BetterQuizzes draft. Required input shape: { draftId, question }. This stores one question and does not open the widget. After the first accepted add_question in the normal flow, call open_quiz immediately before adding question 2. Matching canonical shape: {type:'matching', left:[{id,text}], right:[{id,text}]}; optional grading keys use answer:[{leftId,rightId}]. Legacy pairs/matches/items are normalized.",
