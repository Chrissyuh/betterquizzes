...
const QUESTION_TYPE_GUIDE = "Question answer shapes: multiple_choice answer=zero-based index; multi_select answer=zero-based indexes; true_false answer=boolean; numeric answer=number plus optional tolerance; fill_blank/short_answer answer=string or string[] plus optional acceptableAnswers and optional responseLimit.maxChars. Choice counts should fit the task: 3 answers are fine for simple discrimination, 4 is not mandatory, and 5+ answers are encouraged for richer classification or identification when the extra options are meaningful; avoid filler options. text_select uses segments:[{id,text,selectable?}], optional selectionPolicy, and answer:string[] of selected segment ids. Use text_select only for a passage with context, usually at least two sentences or 120 characters, and at least three plausible selectable segments; do not make one sentence with one obvious highlighted answer. For sentence-selection questions, create contextual text with meaningful selectable segments and plausible distractor segments; do not make the correct segment the only reasonable clickable text. Do not use choices for text_select. Ordering answer=ordered item ids in visual top-to-bottom order with orderingBehavior labels when direction matters; matching uses left:[{id,text}], right:[{id,text}], answer:[{leftId,rightId}], and optional matchingBehavior:{rightItemReuse:'unique'} for one-to-one matching; otherwise right answers can be reused. Do not author matching as pairs unless repairing legacy input. Light formatting includes LaTeX math using \\(...\\) or \\[...\\] and <u>...</u> for critical negations like not, isn't, and except. Do not use dollar-sign math delimiters. Keep compact labels short for mobile.";
