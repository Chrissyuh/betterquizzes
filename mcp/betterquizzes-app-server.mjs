...
const V23_BUILDER_TOOL_DEFS = [
  {
    name: "start_quiz",
    description: "Start a BetterQuizzes draft. For normal assistant-authored quizzes, set expectedQuestionCount, alternate add_question and open_quiz: call add_question exactly once per question, and call open_quiz after each accepted question so the visible widget refreshes. " + V2_BUILDER_INSTRUCTIONS,
  ...
  {
    name: "add_question",
    description: "Add one validated question to an incremental BetterQuizzes draft. Required input shape: { draftId, question }. This stores one question and does not open the widget. In the normal flow, call open_quiz after each accepted add_question so the visible widget refreshes. Matching canonical shape: {type:'matching', left:[{id,text}], right:[{id,text}]}; optional grading keys use answer:[{leftId,rightId}]. Legacy pairs/matches/items are normalized.",
