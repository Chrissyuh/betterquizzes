...
function validateQuizSpec(input) {
	const quiz = input;
	const errors = [];
	const warnings = [];
	if (!quiz || typeof quiz !== "object" || Array.isArray(quiz)) return {
		ok: false,
		errors: ["Quiz must be an object."],
		warnings
	};
	...
	if (![
		...
		"test",
		"survey"
	].includes(String(quiz.mode))) errors.push("mode must be practice, test, or survey.");
	if (!Array.isArray(quiz.questions) || quiz.questions.length === 0) errors.push("questions must be a non-empty array.");
	const ids = /* @__PURE__ */ new Set();
	if (Array.isArray(quiz.questions)) quiz.questions.forEach((rawQuestion, index) => {
		...
		if (question.responseLimit !== void 0) {
			...
			else if (limit && limit.maxChars !== void 0 && limit.maxChars !== null && (!Number.isInteger(limit.maxChars) || Number(limit.maxChars) < 1)) errors.push(`${prefix}.responseLimit.maxChars must be a positive integer, null, or omitted.`);
		}
		...
		if (question.type === "text_select") if (!Array.isArray(question.segments) || question.segments.length < 1) errors.push(`${prefix}.segments must contain at least one segment.`);
	...
	if (![
		...
		"after_submit",
		"never"
	].includes(displayPolicy.showCorrectAnswers)) errors.push("displayPolicy.showCorrectAnswers is invalid.");
	...
	if (![
		...
		"local",
		"hybrid"
	].includes(gradingPolicy.preferredGrader)) errors.push("gradingPolicy.preferredGrader is invalid.");
	...
	if (gradingPolicy.preferredGrader === "llm" && displayPolicy.showCorrectAnswers === "instant") warnings.push("Instant correct answers with an LLM grader may reduce practice value unless intentional.");
...
//#endregion
//#region src/shared/submission.ts
...
function createSession(quiz, answers, startedAt, submittedAt = (/* @__PURE__ */ new Date()).toISOString()) {
	return {
...
function createSubmissionCapsule(quiz, session) {
	const displayPolicy = normalizeDisplayPolicy(quiz.displayPolicy);
	const gradingPolicy = normalizeGradingPolicy(quiz.gradingPolicy);
	const activityPolicy = normalizeActivityPolicy(quiz.activityPolicy);
	...
	return {
		...
		mode: quiz.mode,
		submittedAt: session.submittedAt ?? (/* @__PURE__ */ new Date()).toISOString(),
		displayPolicy,
		gradingPolicy,
		activityPolicy,
		completion,
...
function buildLlmReturnPrompt(submission) {
	return [
...
function buildCompletionSummary(quiz, answers, displayPolicy = normalizeDisplayPolicy(quiz.displayPolicy), activityPolicy = normalizeActivityPolicy(quiz.activityPolicy)) {
	...
	const requiredQuestions = quiz.questions.filter((question) => isQuestionRequired$1(question, activityPolicy));
	const optionalQuestions = quiz.questions.filter((question) => !isQuestionRequired$1(question, activityPolicy));
...
function answerHasResponseForQuestion(question, answer) {
	...
	if (question.type === "matching") {
		...
		return question.left.every((left) => response.some((pair) => pair && typeof pair === "object" && pair.leftId === left.id && rightIds.has(pair.rightId)));
	}
	if (question.type === "multi_typing" || question.type === "multi_write_vertical") {
...
function isTextSelectComplete$1(question, selected) {
	...
	const count = typeof policy.count === "number" && Number.isInteger(policy.count) ? policy.count : void 0;
	const min = typeof policy.min === "number" && Number.isInteger(policy.min) ? policy.min : void 0;
	const max = typeof policy.max === "number" && Number.isInteger(policy.max) ? policy.max : void 0;
	if (policy.mode === "exact_count" || count !== void 0) return cleanSelected.length === (count ?? 1);
	if (policy.mode === "range" || min !== void 0 || max !== void 0) {
		if (min !== void 0 && cleanSelected.length < min) return false;
		if (max !== void 0 && cleanSelected.length > max) return false;
...
function answerHasResponse(answer) {
	...
	const response = answer.response;
	if (response === null || response === void 0) return false;
...
function isQuestionRequired$1(question, activityPolicy) {
	return question.answerRequired ?? question.required ?? activityPolicy.defaultAnswerRequired;
}
function buildLlmInstructions(quiz, displayPolicy, gradingPolicy, activityPolicy) {
	const correctAnswerInstruction = displayPolicy.showCorrectAnswers === "never" ? "Do not reveal the full answer key unless the user explicitly asks after the attempt." : displayPolicy.showCorrectAnswers === "after_submit" ? "After grading, you may show correct answers for missed questions." : "Correct answers were allowed during the activity; still focus feedback on reasoning and confidence.";
	const explanationInstruction = displayPolicy.showExplanations === "llm_after_submit" ? "Provide explanations now, after submission." : "Keep explanations minimal unless the user asks.";
	return [
...
function questionToSnapshot(question) {
	return {
		id: question.id,
		type: question.type,
		...
		...question.type === "multi_typing" ? { multiTypingFields: question.fields.map((field) => ({
			id: field.id,
			label: field.label
		})) } : {},
		...question.type === "multi_write_vertical" ? { multiWriteFields: question.fields.map((field) => ({
			id: field.id,
			label: field.label
		})) } : {},
		...question.type === "text_select" ? {
			textSelectSegments: question.segments.map((segment) => ({
				id: segment.id,
				text: segment.text,
				selectable: segment.selectable
			})),
			textSelectPolicy: question.selectionPolicy
		} : {}
	};
}
function buildAnswerKey(questions) {
	const entries = [];
	for (const question of questions) switch (question.type) {
		case "multiple_choice":
		case "multi_select":
		case "true_false":
		case "fill_blank":
		case "matching":
			if (question.answer !== void 0) entries.push({
		...
		case "multi_write_vertical": {
			const answer = question.answer ?? Object.fromEntries(question.fields.filter((field) => field.answer !== void 0).map((field) => [field.id, field.answer]));
			const expectedKeywords = Object.fromEntries(question.fields.filter((field) => field.expectedKeywords !== void 0).map((field) => [field.id, field.expectedKeywords]));
			if (Object.keys(answer).length || Object.keys(expectedKeywords).length || "rubric" in question && question.rubric) entries.push({
				questionId: question.id,
				answer,
				expectedKeywords: Object.keys(expectedKeywords).length ? expectedKeywords : void 0,
				..."rubric" in question ? { rubric: question.rubric } : {}
			});
			break;
		}
		case "text_select":
			if (question.answer !== void 0 || question.rubric !== void 0) entries.push({
				questionId: question.id,
				answer: question.answer,
		...
		case "ordering":
			if (question.answer !== void 0) entries.push({
				questionId: question.id,
				answer: question.answer,
				rubric: ["Ordering answers are compared in visual top-to-bottom order."]
			});
			break;
		case "numeric":
			if (question.answer !== void 0) entries.push({
			...
			break;
		case "short_answer":
			if (question.answer !== void 0 || question.expectedKeywords !== void 0) entries.push({
				questionId: question.id,
				answer: question.answer,
...
//#endregion
//#region src/shared/compactSubmission.ts
function encodeCompactSubmission(submission) {
	const confidence = submission.answers.filter((answer) => answer.confidence !== void 0).length;
	return [
		"BQS2",
...
var SUPPORTED_QUESTION_TYPES = new Set([
	"multiple_choice",
	"multi_select",
	"true_false",
	"fill_blank",
...
function normalizeQuizForRender(rawQuiz) {
...
function prepareQuizForRender(input) {
	...
	if (![
		...
		"test",
		"survey"
	].includes(String(quiz.mode))) errors.push("quiz.mode must be practice, test, or survey");
	if (!Array.isArray(quiz.questions) || quiz.questions.length === 0) errors.push("quiz.questions must be a non-empty array");
	quiz.displayPolicy = normalizeDisplayPolicy(asPartial(quiz.displayPolicy));
	quiz.gradingPolicy = normalizeGradingPolicy(asPartial(quiz.gradingPolicy));
	quiz.activityPolicy = normalizeActivityPolicy(asPartial(quiz.activityPolicy));
	const ids = /* @__PURE__ */ new Set();
	if (Array.isArray(quiz.questions)) {
		...
		for (const [index, rawQuestion] of normalizedQuestions.entries()) {
			...
			if (!isNonEmptyString(rawQuestion.id)) errors.push(`questions[${index}].id must be a non-empty string.`);
			else if (ids.has(rawQuestion.id)) errors.push(`duplicate question id ${rawQuestion.id}`);
			else ids.add(rawQuestion.id);
			if (!isNonEmptyString(rawQuestion.prompt)) errors.push(`questions[${index}].prompt must be a non-empty string.`);
			if (!SUPPORTED_QUESTION_TYPES.has(String(rawQuestion.type))) errors.push(`questions[${index}].type unsupported: ${String(rawQuestion.type)}`);
		}
	}
	const normalizedQuiz = quiz;
	const schemaValidation = validateQuizSpec(normalizedQuiz);
	if (!schemaValidation.ok) {
		for (const error of schemaValidation.errors) if (!errors.includes(error)) errors.push(error);
	}
...
function validateRenderableQuiz(quiz) {
...
function getRenderDiagnostics(quiz, inheritedWarnings = [], normalizedFields = []) {
	...
	questions.forEach((rawQuestion, index) => {
		...
		if (!isNonEmptyString(rawQuestion.id)) unrenderableQuestions.push({
			index,
			questionId,
			reason: "Missing required field: id."
		});
		if (!isNonEmptyString(rawQuestion.prompt)) unrenderableQuestions.push({
			index,
			questionId,
			reason: "Missing required field: prompt."
		});
		if (!SUPPORTED_QUESTION_TYPES.has(String(rawQuestion.type))) {
			unrenderableQuestions.push({
				index,
				questionId,
				reason: `Unsupported type: ${String(rawQuestion.type)}.`
			});
			return;
		}
		const type = rawQuestion.type;
		componentByQuestion[questionId] = rendererComponentForType(type);
		if ((type === "multiple_choice" || type === "multi_select") && !hasRenderableChoices(rawQuestion.choices)) unrenderableQuestions.push({
			index,
			questionId,
			reason: "Choice question requires choices: string[] with at least one non-empty choice."
		});
		if (type === "multi_typing" && (!Array.isArray(rawQuestion.fields) || rawQuestion.fields.length < 2 || !rawQuestion.fields.every(isRenderableTypingField))) unrenderableQuestions.push({
			index,
			questionId,
			reason: "Multi-typing question requires fields: {id,label}[] with at least two valid fields."
		});
		if (type === "multi_write_vertical" && (!Array.isArray(rawQuestion.fields) || rawQuestion.fields.length < 1 || !rawQuestion.fields.every(isRenderableTypingField))) unrenderableQuestions.push({
			index,
			questionId,
			reason: "Multi-write vertical question requires fields: {id,label}[] with at least one valid field."
		});
		if (type === "text_select" && (!Array.isArray(rawQuestion.segments) || rawQuestion.segments.length < 1 || !rawQuestion.segments.every(isRenderableTextSelectSegment))) unrenderableQuestions.push({
			index,
			questionId,
			reason: "Text-select question requires segments: {id,text,selectable?}[] with at least one valid segment."
		});
		if (type === "matching" && (!Array.isArray(rawQuestion.left) || !Array.isArray(rawQuestion.right) || rawQuestion.left.length < 1 || rawQuestion.right.length < 1 || !rawQuestion.left.every(isRenderableItem) || !rawQuestion.right.every(isRenderableItem))) unrenderableQuestions.push({
			index,
			questionId,
...
function formatRenderContractIssue(result) {
	const lines = ["Quiz setup issue"];
	if (result.errors.length) lines.push("Errors:", ...result.errors.map((error) => `- ${error}`));
	if (result.diagnostics.unrenderableQuestions.length) lines.push("Unrenderable questions:", ...result.diagnostics.unrenderableQuestions.map((item) => `- ${item.questionId}: ${item.reason}`));
...
function normalizeQuestion(raw, index, warnings, normalizedFields) {
	...
	const question = { ...raw };
	if (!question.id && question.questionId !== void 0) {
	...
	if (!question.prompt) {
		const promptAliasName = question.stem !== void 0 ? "stem" : question.question !== void 0 ? "question" : question.text !== void 0 ? "text" : question.title !== void 0 ? "title" : null;
	...
	if (typeof question.type === "string" && QUESTION_TYPE_ALIASES.has(question.type)) {
		...
		normalizedFields.push({
			...
			from: fromType,
			to: String(question.type)
		});
	}
	if (question.answerRequired === void 0 && question.required !== void 0) {
	...
	if (question.answer === void 0) {
		if (question.correctAnswer !== void 0) {
		...
		} else if (question.answerKey !== void 0) {
	...
	if (question.answer === void 0 && Array.isArray(question.acceptedAnswers) && (question.type === "fill_blank" || question.type === "short_answer")) {
	...
	if ((question.type === "multiple_choice" || question.type === "multi_select") && Array.isArray(question.choices)) {
		...
		if (question.type === "multiple_choice" && question.answer !== void 0) question.answer = normalizeChoiceAnswer(question.answer, normalized, warnings, index);
	...
	if (question.type === "text_select") {
		...
		if (!isRecord(raw.selectionPolicy)) normalizedFields.push({
			path: `questions[${index}].selectionPolicy`,
			from: null,
			to: "selectionPolicy"
		});
	}
	if (question.type === "ordering") {
		question.orderingBehavior = normalizeOrderingBehavior(question.orderingBehavior, question.prompt);
		if (!isRecord(raw) || !isRecord(raw.orderingBehavior)) normalizedFields.push({
			path: `questions[${index}].orderingBehavior`,
			from: "prompt",
			to: "orderingBehavior"
		});
	}
...
function normalizeTextSelectSegments(segments) {
...
function normalizeTextSelectPolicy(raw) {
	...
	const count = toPositiveInteger$1(record.count ?? record.selectCount ?? record.requiredSelections);
	const min = toNonNegativeInteger$1(record.min ?? record.minSelections);
	const max = toPositiveInteger$1(record.max ?? record.maxSelections);
	const mode = rawMode === "exact_count" || rawMode === "exact" || count !== void 0 ? "exact_count" : rawMode === "range" || min !== void 0 || max !== void 0 ? "range" : "all_that_apply";
	const instruction = typeof record.instruction === "string" && record.instruction.trim() ? record.instruction.trim() : void 0;
	return {
		mode,
		...count !== void 0 ? { count } : {},
		...min !== void 0 ? { min } : {},
...
function toPositiveInteger$1(value) {
	return typeof value === "number" && Number.isInteger(value) && value > 0 ? value : void 0;
}
function toNonNegativeInteger$1(value) {
	return typeof value === "number" && Number.isInteger(value) && value >= 0 ? value : void 0;
}
	...
	choices.forEach((choice, choiceIndex) => {
		...
		if (isRecord(choice)) {
			...
			if ([
				...
				"number",
				"boolean"
			].includes(typeof text) && String(text).trim()) {
...
function normalizeOrderingBehavior(raw, prompt) {
	if (isRecord(raw)) {
		const topLabel = isNonEmptyString(raw.topLabel) ? raw.topLabel.trim() : void 0;
		const bottomLabel = isNonEmptyString(raw.bottomLabel) ? raw.bottomLabel.trim() : void 0;
	...
	return inferOrderingBehavior$1(typeof prompt === "string" ? prompt : "");
}
function inferOrderingBehavior$1(prompt) {
	const normalized = prompt.toLowerCase();
	if (normalized.includes("greatest to least") || normalized.includes("largest to smallest") || normalized.includes("highest to lowest")) return {
		direction: "top_to_bottom",
		topLabel: "Greatest",
		bottomLabel: "Least"
	};
	if (normalized.includes("least to greatest") || normalized.includes("smallest to largest") || normalized.includes("lowest to highest")) return {
		direction: "top_to_bottom",
		topLabel: "Least",
		bottomLabel: "Greatest"
	};
	if (normalized.includes("oldest to newest")) return {
		direction: "top_to_bottom",
		topLabel: "Oldest",
		bottomLabel: "Newest"
	};
	if (normalized.includes("newest to oldest")) return {
		direction: "top_to_bottom",
		topLabel: "Newest",
		bottomLabel: "Oldest"
	};
	if (normalized.includes("first to last")) return {
		direction: "top_to_bottom",
		topLabel: "First",
		bottomLabel: "Last"
	};
...
function validateAnswerShape(question, questionId, warnings) {
	...
	if ((question.type === "multi_typing" || question.type === "multi_write_vertical") && !(isRecord(question.answer) || question.answer === void 0)) warnings.push(`${questionId}: ${String(question.type)} answer should be a field-id keyed object when provided.`);
	if (question.type === "text_select" && !(Array.isArray(question.answer) || question.answer === void 0)) warnings.push(`${questionId}: text_select answer should be an array of selected segment ids when provided.`);
...
function hasRenderableChoices(value) {
...
function getPersistedSubmissionState(quizId, launchId) {
	const state = getPersistedWidgetStateRecord();
...
async function submitToHost(submission, timeoutMs = 8e3) {
	const bridge = getOpenAiBridge();
...
function asSubmissionCapsule(value) {
	const record = asRecord(value);
	if (!record) return void 0;
	if (record.schema !== "betterquizzer.submission" || record.version !== 2) return void 0;
	if (typeof record.quizId !== "string" || typeof record.sessionId !== "string") return void 0;
	if (!Array.isArray(record.answers) || !Array.isArray(record.questions)) return void 0;
	return record;
}
function asQuizSession(value) {
	const record = asRecord(value);
	if (!record) return void 0;
	if (record.schema !== "betterquizzer.session" || record.version !== 2) return void 0;
	if (typeof record.quizId !== "string" || typeof record.sessionId !== "string") return void 0;
	if (typeof record.startedAt !== "string" || !Array.isArray(record.answers)) return void 0;
	return record;
}
function asToolResultLike(value) {
...
function isCompleteRenderableLaunch(summaryRecord, quiz, diagnostics, source) {
	const expectedQuestionCount = getExpectedQuestionCount(summaryRecord ?? null);
	if (!(diagnostics.rendererCertified && diagnostics.renderableQuestionCount === diagnostics.questionCount && diagnostics.questionCount === quiz.questions.length)) return false;
...
function toUploadProgress(summary, quiz, diagnostics) {
	const rawProgress = asRecord(summary?.packetProgress) ?? asRecord(summary?.uploadProgress);
	const expectedQuestions = typeof rawProgress?.expectedQuestions === "number" && Number.isFinite(rawProgress.expectedQuestions) ? rawProgress.expectedQuestions : getExpectedQuestionCount(summary ?? null) ?? quiz.questions.length;
	const receivedQuestions = typeof rawProgress?.receivedQuestions === "number" && Number.isFinite(rawProgress.receivedQuestions) ? rawProgress.receivedQuestions : quiz.questions.length;
	return {
		expectedQuestions,
		receivedQuestions,
		renderableQuestions: typeof rawProgress?.renderableQuestions === "number" && Number.isFinite(rawProgress.renderableQuestions) ? rawProgress.renderableQuestions : diagnostics.renderableQuestionCount,
...
function asQuiz(value) {
	const record = asRecord(value);
	if (record?.schema === "betterquizzer.quiz" && record.version === 2 && Array.isArray(record.questions)) return record;
	return null;
}
var tiny_demo_default = {
	schema: "betterquizzer.quiz",
	version: 2,
	quizId: "tiny-demo-v2",
	title: "Tiny BetterQuizzes v2 Demo",
	description: "A tiny quiz used to test the neutral LLM-controlled submission flow.",
	subject: "General",
	mode: "practice",
	displayPolicy: {
		"showCorrectAnswers": "after_submit",
	...
	questions: [
		...
		},
		{
			"id": "q2",
			"type": "fill_blank",
			"prompt": "The capital of France is ________.",
			"answer": ["Paris"],
			"tags": ["geography"],
			"difficulty": 1
		},
		{
			"id": "q3",
			"type": "true_false",
			"prompt": "BetterQuizzes v2 sends structured submissions back to an LLM for grading and teaching.",
			"answer": true,
			"tags": ["betterquizzer", "workflow"],
			"difficulty": 1
		}
	],
	metadata: {
...
var aphg_demo_default = {
	schema: "betterquizzer.quiz",
	version: 2,
	quizId: "aphg-agriculture-v2",
	title: "APHG Agriculture Review v2",
	description: "A short AP Human Geography activity using the LLM-controlled BetterQuizzes v2 flow.",
	...
	questions: [
		{
			"id": "q1",
			"type": "multiple_choice",
			"prompt": "Which model explains agricultural land use around a central market?",
			"choices": [
				"Rostow",
				"Von Thünen",
				"Malthus",
		...
		},
		{
			"id": "q2",
			"type": "multi_select",
			"prompt": "Select the factors that can influence agricultural land use intensity.",
			"choices": [
				"Transportation cost",
				"Distance to market",
				"Soil fertility",
		...
		},
		{
			"id": "q3",
			"type": "fill_blank",
			"prompt": "Growing only one crop over a large area is called ________.",
			"answer": [
				"monoculture",
				"mono culture",
				"monocropping"
			],
			"tags": ["agriculture", "vocabulary"],
			"difficulty": 2
		},
		{
			"id": "q4",
			"type": "short_answer",
			"prompt": "In one sentence, explain why perishable crops are usually grown closer to a market in the Von Thünen model.",
			"answer": "Perishable crops are grown closer because spoilage and transport time make long-distance travel more costly.",
			"expectedKeywords": [
				"perishable",
				"spoil",
				"transport",
		...
		},
		{
			"id": "q5",
			"type": "numeric",
			"prompt": "A farmer earns $500 revenue and has $120 in transportation costs. What is the net return?",
			"answer": 380,
			"tolerance": 0,
			"unit": "dollars",
			"tags": [
...
var mixed_types_default = {
	schema: "betterquizzer.quiz",
	version: 2,
	quizId: "mixed-types-v2-demo",
	title: "Mixed Question Types v2 Demo",
	description: "A broad UI test for BetterQuizzes v2 question types.",
	subject: "Demo",
	mode: "practice",
	displayPolicy: {
		"showCorrectAnswers": "after_submit",
	...
	questions: [
		{
			"id": "q1",
			"type": "multiple_choice",
			"prompt": "Which file extension should BetterQuizzes quiz files use?",
			"choices": [
				".txt",
				".bqz",
				".png",
			...
			"answer": 1,
			"tags": ["betterquizzer", "files"],
			"difficulty": 1
		},
		{
			"id": "q2",
			"type": "ordering",
			"prompt": "Put the BetterQuizzes flow in order.",
			"items": [
				{
					"id": "import",
					"text": "Import QuizSpec"
				},
				{
					"id": "answer",
					"text": "User answers questions"
				},
				{
					"id": "submit",
					"text": "Create SubmissionCapsule"
				},
				{
					"id": "grade",
					"text": "LLM grades and teaches"
				}
			],
			"answer": [
				"import",
		...
		},
		{
			"id": "q3",
			"type": "matching",
			"prompt": "Match each BetterQuizzes object to its job.",
			"left": [
				{
					"id": "quizspec",
					"text": "QuizSpec"
				...
				{
					"id": "submission",
					"text": "SubmissionCapsule"
				}
			],
			"right": [
				{
					"id": "wholequiz",
					"text": "The quiz the LLM creates"
				},
				{
					"id": "singleanswer",
					"text": "One response plus confidence"
				},
				{
					"id": "llmreturn",
					"text": "The object sent back for grading"
				}
			],
			"answer": [
				{
			...
			"tags": ["betterquizzer", "schema"],
			"difficulty": 2
		},
		{
			"id": "q4",
			"type": "long_response",
			"prompt": "Explain one reason BetterQuizzes should let the LLM do explanations instead of embedding explanations into the quiz UI.",
			"rubric": [
				"Mentions that the LLM is the teacher/tutor",
				"Explains that the UI should stay neutral and flexible",
				"Connects answer feedback to targeted follow-up teaching"
			],
			"tags": [
				"betterquizzer",
				"product-design",
		...
		},
		{
			"id": "q5",
			"type": "numeric",
			"prompt": "How many answers does a true/false question have?",
			"answer": 2,
			"tolerance": 0,
			"tags": ["betterquizzer", "numeric"],
			"difficulty": 1
		}
	],
	metadata: {
...
function App() {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(QuizRunner, {
		quiz,
		startedAt,
		launchId,
		widgetMode,
...
function formatWidgetVersion(version) {
...
function hasBetterQuizzesBootstrap() {
	if (typeof window === "undefined") return false;
...
async function fetchQuizFromServer(quizId) {
	...
	if (!response.ok) {
		const detail = body && typeof body === "object" && "error" in body ? String(body.error) : response.status + " " + response.statusText;
...
function inferRestoredStartedAt(submission) {
	const submittedMs = Date.parse(submission.submittedAt);
	if (!Number.isFinite(submittedMs)) return submission.submittedAt || (/* @__PURE__ */ new Date()).toISOString();
...
function getRestoredDeliveryStatus(state, submission) {
	if (submission.status?.followUpRequested) return "grade_requested";
	switch (state.status) {
		case "grade_requested": return "grade_requested";
		case "requesting_grade": return "requesting_grade";
		case "retrying_grade_request": return "retrying_grade_request";
		case "fallback_ready": return "grade_request_unavailable";
		case "failed": return "grade_request_failed";
		case "submitting": return "requesting_grade";
		default: return "submitted";
	}
}
function toHydrationProgress(payload, quiz) {
...
function isMoreCompleteQuiz(candidate, current) {
	...
	return countRenderableQuestionFields(candidate) > countRenderableQuestionFields(current);
}
function countRenderableQuestionFields(quiz) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("main", {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("section", {
				...
				children: SAMPLE_QUIZZES.map((sample) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
					className: "sample-card",
					type: "button",
					onClick: () => onLoadQuiz(sample),
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
							className: "badge",
							children: "Sample"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: sample.title }),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: sample.description ?? sample.subject ?? "Demo activity" }),
...
function QuizRunner({ quiz, startedAt, launchId, widgetMode, onReset, onFinish }) {
	...
	function updateDraft(questionId, patch) {
		setDrafts((previous) => {
			...
			quiz.questions.find((item) => item.id === questionId);
			const merged = {
				...previousDraft,
				...patch,
	...
	function exitWithoutGrading() {
		if (widgetMode) persistSubmissionState({
			status: "skipped",
			quizId: getQuizId(quiz),
			launchId,
			currentIndex,
	...
	async function finish(options = {}) {
		...
		try {
			if (widgetMode) {
				try {
					hostResult = await submitToHost(submission, 8e3);
				...
				submission.status = {
					...
					hostSubmitted,
					warnings: uniqueWarnings([...submission.status?.warnings ?? [], ...bridgeError ? [`Answers were saved locally, but the host submit bridge reported: ${bridgeError}`] : []])
				};
				persistSubmissionState({
			...
			if (widgetMode) requestChatGptGradeOnce({
				submission,
				session,
				hostSubmitted,
				currentIndex,
	...
	if (skipMode) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SkipQuizScreen, {
		mode: skipMode,
		widgetMode,
		title: quiz.title,
		answeredCount,
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
				...
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							...
							children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
								className: "eyebrow",
								children: "BetterQuizzes"
							}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
								className: "version-chip",
								children: WIDGET_VERSION_LABEL
							})]
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RichInline, { text: quiz.title }) }),
						quiz.description ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RichBlock, { text: quiz.description }) : null
					] }),
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: "top-actions",
						children: [!widgetMode ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							type: "button",
							onClick: onReset,
							children: "New quiz"
						}) : null, activityPolicy.allowSkipQuiz ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							className: "skip-quiz-button",
							type: "button",
							onClick: () => skipQuiz(),
							children: "Skip"
						}) : null]
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
						...
						children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { style: { width: `${progressPercent}%` } })
					})
				]
			}),
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
				...
				children: [
					...
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						className: `actions split compact-actions ${quiz.questions.length <= 1 ? "single-question-actions" : ""}`,
						children: [quiz.questions.length > 1 ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
							className: "actions",
...
function SkipQuizScreen({ mode, widgetMode, title, answeredCount, totalCount, submitting, onResume, onSubmitAnswered, onExitWithoutGrading, onNewQuiz }) {
	if (mode === "skipped") return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("main", {
		className: "shell narrow result-shell",
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
			className: "card result-hero stack",
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "eyebrow eyebrow-row",
					children: ["Quiz skipped ", /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "version-chip",
						children: WIDGET_VERSION_LABEL
					})]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: "No grade was created" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", { children: [
					"You exited “",
					title,
					"” before submitting answers for grading."
				] }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("main", {
		...
		children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
			...
			children: [
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
					className: "eyebrow eyebrow-row",
					children: ["Leave quiz? ", /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
						className: "version-chip",
						children: WIDGET_VERSION_LABEL
					})]
				}),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: answeredCount > 0 ? "Submit what you answered?" : "Exit without grading?" }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: answeredCount > 0 ? `You have answered ${answeredCount} of ${totalCount} questions. You can submit those answers for grading, resume the quiz, or exit without creating a grade.` : "You have not answered anything yet. You can resume the quiz or exit without creating a grade." }),
				/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
					className: "actions wrap",
					children: [
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							...
							children: submitting ? "Submitting…" : "Submit answered questions"
						}),
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							type: "button",
							onClick: onResume,
						...
						/* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
							type: "button",
							onClick: onExitWithoutGrading,
							children: "Exit without grading"
						})
					]
				})
			]
...
function QuestionNav({ questions, drafts, currentIndex, displayPolicy, activityPolicy, onSelect }) {
...
function QuestionCard({ question, draft, displayPolicy, activityPolicy, quizChoiceBehavior, onChange }) {
		...
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				children: !required ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: "question-meta-label",
					children: "Optional"
				}) : null
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h2", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RichInline, { text: question.prompt }) }),
...
function QuestionInput({ question, draft, quizChoiceBehavior, onChange }) {
	const response = draft?.response;
	switch (question.type) {
		case "multiple_choice": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ChoiceList, {
			question,
			quizChoiceBehavior,
			response,
			onChange: (nextResponse) => onChange({ response: nextResponse })
		});
		case "multi_select": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(MultiSelectList, {
			question,
			quizChoiceBehavior,
			response,
			onChange: (nextResponse) => onChange({ response: nextResponse })
		});
		case "true_false": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TrueFalseList, {
			selected: typeof response === "boolean" ? response : null,
			onSelect: (value) => onChange({ response: value })
		});
		case "fill_blank": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TextField, {
			label: "Your answer",
		...
		case "multi_typing": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(MultiTypingInput, {
			question,
			response: isMultiTypingResponse(response) ? response : {},
			onChange: (value) => onChange({ response: value })
		});
		case "multi_write_vertical": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(MultiWriteVerticalInput, {
			question,
			response: isMultiTypingResponse(response) ? response : {},
			onChange: (value) => onChange({ response: value })
		});
		case "text_select": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TextSelectInput, {
			question,
			response: Array.isArray(response) ? response.filter((item) => typeof item === "string") : [],
			onChange: (value) => onChange({ response: value })
		});
		case "numeric": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TextField, {
			label: question.unit ? `Number (${question.unit})` : "Number",
			inputMode: "text",
			value: typeof response === "number" || typeof response === "string" ? String(response) : "",
			onChange: (value) => onChange({ response: value })
		});
		case "ordering": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(OrderingInput, {
			question,
			response: Array.isArray(response) ? response.filter((item) => typeof item === "string") : [],
			onChange: (value) => onChange({ response: value })
		});
		case "matching": return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(MatchingInput, {
			question,
			response: isMatchingPairs(response) ? response : [],
			onChange: (value) => onChange({ response: value })
		});
		default: return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(QuestionRenderWarning, {
			question,
			detail: "This question has a missing or unsupported type."
		});
	}
}
function QuestionRenderWarning({ question, detail }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
		className: "question-render-warning",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "Question setup issue" }),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: detail }),
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
				className: "muted compact-status",
				children: [
					"questionId: ",
					safeText(question.id) || "(missing)",
					" • type: ",
...
function getChoiceTexts(question) {
...
function RichInline({ text }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
		className: "rich-text",
		children: renderInlineMarkup(safeText(text), "ri")
	});
}
function RichBlock({ text }) {
	const paragraphs = safeText(text).split(/\n{2,}/).map((paragraph) => paragraph.trim()).filter(Boolean);
	if (!paragraphs.length) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {});
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_jsx_runtime.Fragment, { children: paragraphs.map((paragraph, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
		className: "rich-text rich-block",
		children: renderInlineMarkup(paragraph, `rb-${index}`)
	}, index)) });
}
function renderInlineMarkup(text, keyPrefix) {
	...
	while ((match = tokenPattern.exec(text)) !== null) {
		...
		if (token.startsWith("**") && token.endsWith("**")) nodes.push(/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: renderInlineMarkup(token.slice(2, -2), `${key}-b`) }, key));
		else if (/^<u>/i.test(token)) nodes.push(/* @__PURE__ */ (0, import_jsx_runtime.jsx)("u", { children: renderInlineMarkup(token.slice(3, -4), `${key}-u`) }, key));
		else if (/^<sub>/i.test(token)) nodes.push(/* @__PURE__ */ (0, import_jsx_runtime.jsx)("sub", { children: renderInlineMarkup(token.slice(5, -6), `${key}-sub`) }, key));
		else if (/^<sup>/i.test(token)) nodes.push(/* @__PURE__ */ (0, import_jsx_runtime.jsx)("sup", { children: renderInlineMarkup(token.slice(5, -6), `${key}-sup`) }, key));
		else if (token.startsWith("`") && token.endsWith("`")) nodes.push(/* @__PURE__ */ (0, import_jsx_runtime.jsx)("code", { children: token.slice(1, -1) }, key));
		else if (token.startsWith("*") && token.endsWith("*")) nodes.push(/* @__PURE__ */ (0, import_jsx_runtime.jsx)("em", { children: renderInlineMarkup(token.slice(1, -1), `${key}-i`) }, key));
...
function appendTextWithBreaks(nodes, text, keyPrefix) {
	text.split(/\n/).forEach((part, index) => {
		if (index > 0) nodes.push(/* @__PURE__ */ (0, import_jsx_runtime.jsx)("br", {}, `${keyPrefix}-br-${index}`));
		if (part) nodes.push(part);
	});
}
function getOrderingItems(question) {
	const record = question && typeof question === "object" ? question : {};
	if (!Array.isArray(record.items)) return [];
	return record.items.filter((item) => Boolean(item) && typeof item === "object" && "id" in item && "text" in item).map((item) => ({
		id: String(item.id),
...
function getInitialOrderingOrder(question, items = getOrderingItems(question)) {
	...
	const record = question && typeof question === "object" ? question : {};
	const answer = Array.isArray(record.answer) ? record.answer.filter((id) => typeof id === "string") : [];
...
function normalizeOrderingResponse(response, items) {
	...
	return [...response.filter((id) => {
		...
		seen.add(id);
		return true;
	}), ...itemIds.filter((id) => !seen.has(id))];
}
function getMatchingSide(question, side) {
	const raw = side === "left" ? question.left : question.right;
	if (!Array.isArray(raw)) return [];
	return raw.filter((item) => Boolean(item) && typeof item === "object" && "id" in item && "text" in item).map((item) => ({
		id: String(item.id),
		text: String(item.text)
	})).filter((item) => item.id.trim().length > 0 && item.text.trim().length > 0);
}
function getMultiTypingFields(question) {
	if (!Array.isArray(question.fields)) return [];
	return question.fields.filter((field) => Boolean(field) && typeof field === "object" && "id" in field && "label" in field).map((field) => ({
		id: String(field.id),
...
function getFieldResponseLimit(raw) {
	if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
	const record = raw;
	return {
		maxChars: record.maxChars === null ? null : typeof record.maxChars === "number" && Number.isFinite(record.maxChars) ? Math.max(0, Math.floor(record.maxChars)) : void 0,
		minChars: typeof record.minChars === "number" && Number.isFinite(record.minChars) ? Math.max(0, Math.floor(record.minChars)) : void 0,
		showCounter: typeof record.showCounter === "boolean" ? record.showCounter : void 0
	};
}
function isMultiTypingResponse(value) {
	return Boolean(value) && typeof value === "object" && !Array.isArray(value) && !("kind" in value) && Object.values(value).every((item) => typeof item === "string");
}
function getTextSelectSegments(question) {
	if (!Array.isArray(question.segments)) return [];
	return question.segments.flatMap((segment, index) => {
		...
		return [{
			id,
			text,
			selectable: record.selectable === false ? false : true
		}];
	});
}
function getTextSelectPolicy(question) {
...
function getTextSelectInstruction(policy) {
	...
	if (policy.mode === "range") {
		if (policy.min !== void 0 && policy.max !== void 0) return "Select " + policy.min + " to " + policy.max + " segments.";
		if (policy.min !== void 0) return "Select at least " + policy.min + " segment" + (policy.min === 1 ? "." : "s.");
		if (policy.max !== void 0) return "Select up to " + policy.max + " segment" + (policy.max === 1 ? "." : "s.");
	}
	return "Select all that apply.";
}
function isTextSelectComplete(question, response) {
	if (!Array.isArray(response) || !response.every((item) => typeof item === "string")) return false;
	...
	if (policy.mode === "range") {
		if (policy.min !== void 0 && selected.length < policy.min) return false;
		if (policy.max !== void 0 && selected.length > policy.max) return false;
		return selected.length > 0 || policy.min === 0;
	}
	return selected.length > 0;
}
function sanitizeTextSelectResponse(question, response) {
...
function ChoiceList({ question, quizChoiceBehavior, response, onChange }) {
	const selected = typeof response === "number" ? response : null;
	const special = asSpecialResponse(response);
	const behavior = {
		...quizChoiceBehavior ?? {},
		...question.choiceBehavior ?? {}
	};
	const allowOther = behavior.allowOther ?? false;
	const choices = getChoiceTexts(question);
	if (!choices.length && !allowOther) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(QuestionRenderWarning, {
		question,
		detail: "This multiple-choice question did not include any valid choices."
	});
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		...
		children: [choices.map((choice, index) => {
			...
			return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
				type: "button",
				className: selectedChoice ? "choice single-choice selected" : "choice single-choice",
				onClick: () => onChange(index),
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
					className: "choice-letter",
					"aria-hidden": "true",
					children: label
				}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(RichInline, { text: choice }) })]
			}, String(index));
		}), allowOther ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
			className: special?.kind === "other" ? "choice other-choice selected" : "choice other-choice",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "choice-letter",
				children: "+"
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
				value: special?.kind === "other" ? special.text : "",
				placeholder: behavior.otherLabel ?? "Other...",
				onFocus: () => onChange({
					kind: "other",
					text: special?.kind === "other" ? special.text : ""
				}),
...
function MultiSelectList({ question, quizChoiceBehavior, response, onChange }) {
	const selected = Array.isArray(response) ? response.filter((item) => typeof item === "number") : [];
	const special = asSpecialResponse(response);
	const behavior = {
		...quizChoiceBehavior ?? {},
		...question.choiceBehavior ?? {}
	...
	if (!choices.length && !behavior.allowOther) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(QuestionRenderWarning, {
		question,
		detail: "This multi-select question did not include any valid choices."
	});
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "choice-list",
		children: [choices.map((choice, index) => {
			...
			return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
				type: "button",
				className: selectedChoice ? "choice multi-choice selected" : "choice multi-choice",
		...
		}), behavior.allowOther ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
			className: special?.kind === "other" ? "choice multi-choice other-choice selected" : "choice multi-choice other-choice",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "choice-letter",
				children: "+"
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
...
function TrueFalseList({ selected, onSelect }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "choice-list",
		children: [/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
			type: "button",
			className: selected === true ? "choice selected" : "choice",
			onClick: () => onSelect(true),
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "choice-letter",
				children: "T"
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "True" })]
		}), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("button", {
			type: "button",
			className: selected === false ? "choice selected" : "choice",
			onClick: () => onSelect(false),
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
				className: "choice-letter",
				children: "F"
			}), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: "False" })]
		})]
	});
}
function TextField({ label, value, onChange, placeholder, inputMode, responseLimit, formatting = false }) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("input", {
				...
				placeholder,
				onChange: (event) => onChange(clampTextToLimit(event.currentTarget.value, maxChars))
			}),
			formatting ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TextFormatToolbar, { onFormat: apply }) : null,
			shouldShowCharCounter(responseLimit, maxChars) ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("span", {
...
function TextArea({ label, value, onChange, rows = 4, responseLimit, formatting = false, placeholder = "Write your answer..." }) {
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
		...
		children: [
			...
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("textarea", {
				ref: textAreaRef,
				className: "text-area no-horizontal-resize auto-text-area",
				rows,
				value: limitedValue,
				maxLength: maxChars ?? void 0,
				placeholder,
...
function toMathAlphanumeric(char, style) {
	...
	if (style === "bold") {
		if (code >= 65 && code <= 90) return String.fromCodePoint(119808 + code - 65);
		if (code >= 97 && code <= 122) return String.fromCodePoint(119834 + code - 97);
		if (code >= 48 && code <= 57) return String.fromCodePoint(120782 + code - 48);
		return char;
	}
	if (code >= 65 && code <= 90) return String.fromCodePoint(119860 + code - 65);
	if (char === "h") return "ℎ";
	if (code >= 97 && code <= 122) return String.fromCodePoint(119886 + code - 97);
	return char;
}
var SUBSCRIPT_MAP = {
	"0": "₀",
...
function MultiTypingInput({ question, response, onChange }) {
	...
	if (fields.length < 2) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(QuestionRenderWarning, {
		question,
		detail: "This multi-typing question needs at least two fields."
	});
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		className: "multi-typing-grid",
		children: fields.map((field) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TextField, {
			label: field.label,
...
function MultiWriteVerticalInput({ question, response, onChange }) {
	const fields = getMultiTypingFields(question);
	if (fields.length < 1) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(QuestionRenderWarning, {
		question,
		detail: "This multi-write vertical question needs at least one field."
	});
	function updateField(fieldId, value) {
		onChange({
			...response,
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		className: "multi-write-vertical",
		children: fields.map((field) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(TextArea, {
			label: field.label,
			value: typeof response[field.id] === "string" ? response[field.id] : "",
...
function TextSelectInput({ question, response, onChange }) {
	const segments = getTextSelectSegments(question);
	const selectableIds = new Set(segments.filter((segment) => segment.selectable !== false).map((segment) => segment.id));
	const selected = response.filter((id) => selectableIds.has(id));
	const policy = getTextSelectPolicy(question);
	...
	if (!segments.length) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(QuestionRenderWarning, {
		question,
		detail: "This text-select question needs at least one text segment."
	});
	...
	function renderSegment(segment, labelText) {
		const selectable = segment.selectable !== false;
		const isSelected = selected.includes(segment.id);
		const label = labelText ?? segment.text;
		if (!selectable) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", {
			className: "text-segment static-segment",
			children: label
		}, segment.id);
		return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", {
			type: "button",
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "text-select-shell",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", {
				className: "text-select-instruction",
				children: getTextSelectInstruction(policy)
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "text-select-content inline-text-select",
				children: inline.nodes.length ? inline.nodes : fallbackSegments.map((segment) => renderSegment(segment))
			}),
			Boolean(inline.nodes.length && fallbackSegments.length) ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				className: "text-select-fallback",
				"aria-label": "Additional selectable segments",
				children: fallbackSegments.map((segment) => renderSegment(segment))
			}) : null,
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
				className: "text-select-count",
				children: [
					"Selected ",
					selected.length,
...
function buildTextSelectInlineParts(text, segments, renderSegment) {
	const renderedSegmentIds = /* @__PURE__ */ new Set();
	if (!text.trim()) return {
		nodes: [],
		renderedSegmentIds
	};
	const lowerText = text.toLowerCase();
	const rawMatches = segments.map((segment, order) => {
	...
	}).filter((match) => Boolean(match)).sort((a, b) => a.index - b.index || b.length - a.length || a.order - b.order);
	const nodes = [];
	let cursor = 0;
...
function getResponseLimit(question) {
	const raw = question.responseLimit;
	if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
	const record = raw;
	return {
		maxChars: record.maxChars === null ? null : typeof record.maxChars === "number" && Number.isFinite(record.maxChars) ? Math.max(0, Math.floor(record.maxChars)) : void 0,
...
function getMaxChars(limit) {
	if (!limit) return null;
	if (limit.maxChars === null || limit.maxChars === void 0) return null;
	return Number.isFinite(limit.maxChars) && limit.maxChars > 0 ? Math.floor(limit.maxChars) : null;
}
function clampTextToLimit(value, maxChars) {
	return maxChars === null ? value : value.slice(0, maxChars);
}
function shouldShowCharCounter(limit, maxChars) {
...
function OrderingInput({ question, response, onChange }) {
	...
	const dragVisualOffsetRef = (0, import_react.useRef)(null);
	...
	function setOrderingDragVisualOffset(offset) {
	...
	function moveToIndex(sourceId, rawIndex) {
		const currentOrder = orderRef.current.length ? orderRef.current : order;
		if (currentOrder.indexOf(sourceId) < 0) return;
	...
	function insertionIndexFromPoint(_clientX, clientY, sourceId) {
		const list = listRef.current;
		const currentOrder = orderRef.current.length ? orderRef.current : order;
		if (!list) return currentOrder.indexOf(sourceId);
		const rows = Array.from(list.querySelectorAll("[data-order-id]")).filter((row) => row.dataset.orderId !== sourceId);
		const listRect = list.getBoundingClientRect();
		for (let index = 0; index < rows.length; index += 1) {
			const row = rows[index];
			if (clientY < listRect.top + row.offsetTop - list.scrollTop + row.offsetHeight / 2) return index;
		}
		return rows.length;
	}
	function clearMobileDocumentDragListeners() {
		mobileDocumentCleanupRef.current?.();
		mobileDocumentCleanupRef.current = null;
		if (dragRef.current) dragRef.current.cleanup = void 0;
	}
	function installMobileDocumentDragListeners() {
		...
		const findActiveTouch = (touches, touchIdentifier) => Array.from(touches).find((candidate) => candidate.identifier === touchIdentifier);
		const onDocumentPointerMove = (event) => {
			const active = dragRef.current;
			if (!active || active.mode !== "mobile" || active.touchIdentifier !== void 0 || active.pointerId !== event.pointerId) return;
		...
		const onDocumentTouchMove = (event) => {
			const active = dragRef.current;
			if (!active || active.mode !== "mobile" || active.touchIdentifier === void 0) return;
		...
		const onDocumentTouchEnd = (event) => {
			const active = dragRef.current;
			if (!active || active.mode !== "mobile" || active.touchIdentifier === void 0) return;
			if (Array.from(event.changedTouches).some((touch) => touch.identifier === active.touchIdentifier)) finishDrag();
		};
		const onDocumentTouchCancel = (event) => {
			const active = dragRef.current;
			if (!active || active.mode !== "mobile" || active.touchIdentifier === void 0) return;
			if (Array.from(event.changedTouches).some((touch) => touch.identifier === active.touchIdentifier)) finishDrag();
		};
	...
	function beginTouchDrag(event, id) {
		if (inputMode !== "mobile") return;
		if (dragRef.current) return;
		if (event.touches.length !== 1) return;
		const touch = event.touches[0];
		startDrag(id, "mobile", touch.clientX, touch.clientY, void 0, touch.identifier);
		event.preventDefault();
	}
	function beginMobilePointerFallbackDrag(event, id) {
		if (inputMode !== "mobile") return;
	...
	function moveActiveDrag(clientX, clientY, event) {
		const active = dragRef.current;
		if (!active) return;
	...
	function moveDrag(event) {
		const active = dragRef.current;
		if (!active || active.pointerId === void 0 || active.pointerId !== event.pointerId) return;
		moveActiveDrag(event.clientX, event.clientY, event);
	}
	function finishDrag() {
		clearMobileDocumentDragListeners();
	...
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "order-shell drag-order-shell bq-ordering-rebuilt",
		"data-ordering-mode": inputMode,
		"aria-label": "Ordering answer",
		children: [
			/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
				className: "order-end-label top-label",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "Top" }),
					" = ",
					behavior.topLabel
				]
			}),
			/* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
				...
				children: order.map((id, index) => {
					...
					return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
						...
						onPointerDown: inputMode === "desktop" ? (event) => beginDrag(event, id, "desktop") : void 0,
...
function MatchingInput({ question, response, onChange }) {
	...
	if (!leftItems.length || !rightItems.length) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(QuestionRenderWarning, {
		question,
		detail: "This matching question needs valid left and right item lists."
	});
	return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", {
		className: "match-list",
		children: leftItems.map((left) => /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("label", {
			className: "match-row",
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: left.text }), /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("select", {
				value: response.find((pair) => pair.leftId === left.id)?.rightId ?? "",
				onChange: (event) => setPair(left.id, event.currentTarget.value),
				children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
					value: "",
					children: "Choose match..."
				}), rightItems.map((right) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("option", {
					value: right.id,
					children: right.text
				}, right.id))]
			})]
...
function ConfidencePicker({ value, required, disabled, onChange }) {
	return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", {
		className: "confidence-picker three-level",
		...
		children: [[
			...
			children: [/* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: item }), /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { children: confidenceLabel(item) })]
		}, item)), required && !disabled && value === void 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", {
			className: "required-note",
			children: "Required"
		}) : null]
	});
}
function SubmissionScreen({ finished, widgetMode, onNewQuiz }) {
...
function summarizeToolResult(result) {
	...
	const text = Array.isArray(result.content) ? result.content.map((item) => {
	...
	return {
		...
		structuredContent: result.structuredContent ? { ok: true } : void 0
	};
}
async function requestChatGptGradeOnce(options) {
	setFollowUpDelivery(options, "requesting_grade", false, 0, "Asking ChatGPT to grade…");
	const result = await sendSubmissionFollowUp(buildAutoGradePrompt(options.submission), 8e3);
...
function setFollowUpDelivery(options, status, followUpRequested, attempts, message) {
	options.submission.status = {
		...
		hostSubmitted: options.hostSubmitted,
		followUpRequested,
		warnings: uniqueWarnings([...options.submission.status?.warnings ?? [], ...message && status !== "grade_requested" && status !== "requesting_grade" && status !== "retrying_grade_request" ? [message] : []])
	};
	persistSubmissionState({
		...
		error: status === "grade_request_failed" || status === "grade_request_unavailable" ? message : void 0
	});
	options.onUpdate({
		submission: options.submission,
		session: options.session,
...
function sanitizeRestoredDrafts(value, quiz) {
	if (!value || typeof value !== "object" || Array.isArray(value)) return {};
	return keepDraftsForQuiz(value, quiz);
}
function keepDraftsForQuiz(drafts, quiz) {
	...
	for (const [questionId, draft] of Object.entries(drafts)) {
		...
		kept[questionId] = sanitizeDraftForQuestion(question, draft);
	}
	return kept;
}
function isDraftAnswerLike(value) {
	return Boolean(value) && typeof value === "object" && !Array.isArray(value) && typeof value.firstSeenAt === "number";
}
function sanitizeDraftForQuestion(question, draft) {
	const base = {
		response: null,
		confidence: normalizeConfidence(draft.confidence),
		firstSeenAt: Number.isFinite(draft.firstSeenAt) ? draft.firstSeenAt : Date.now(),
		lastUpdatedAt: typeof draft.lastUpdatedAt === "number" && Number.isFinite(draft.lastUpdatedAt) ? draft.lastUpdatedAt : void 0
	};
	const response = draft.response;
	switch (question.type) {
		case "multiple_choice": return {
			...base,
			response: typeof response === "number" ? response : asSpecialResponse(response) ?? null
		};
		case "multi_select": return {
			...base,
			response: Array.isArray(response) ? response.filter((item) => typeof item === "number") : asSpecialResponse(response) ?? null
		};
		case "true_false": return {
			...base,
			response: typeof response === "boolean" ? response : null
		};
		case "fill_blank":
		case "short_answer":
		case "long_response": return {
			...base,
			response: typeof response === "string" ? clampTextToLimit(response, getMaxChars(getResponseLimit(question))) : null
		};
		case "multi_typing":
		case "multi_write_vertical": return {
			...base,
			response: sanitizeMultiTypingResponse(question, response)
		};
		case "text_select": return {
			...base,
			response: sanitizeTextSelectResponse(question, response)
		};
		case "numeric": return {
			...base,
			response: typeof response === "number" && Number.isFinite(response) ? response : typeof response === "string" ? response : null
		};
		case "ordering": {
			const restored = Array.isArray(response) ? response.filter((item) => typeof item === "string") : [];
			return {
				...base,
				response: restored.length ? normalizeOrderingResponse(restored, getOrderingItems(question)) : null
			};
		}
		case "matching": return {
			...base,
			response: sanitizeMatchingPairs(question, response)
		};
		default: return base;
	}
}
function sanitizeMultiTypingResponse(question, response) {
	const fields = getMultiTypingFields(question);
...
function sanitizeMatchingPairs(question, response) {
...
function buildAnswerMeta(question) {
	if (question.type === "ordering") {
		const behavior = getOrderingBehavior(question);
		return {
...
function getOrderingBehavior(question) {
	const raw = question.orderingBehavior;
	if (raw && typeof raw === "object" && !Array.isArray(raw)) {
		const record = raw;
		const topLabel = typeof record.topLabel === "string" && record.topLabel.trim() ? record.topLabel.trim() : void 0;
		const bottomLabel = typeof record.bottomLabel === "string" && record.bottomLabel.trim() ? record.bottomLabel.trim() : void 0;
		if (topLabel && bottomLabel) return {
			direction: "top_to_bottom",
			topLabel,
			bottomLabel
...
function makeAnswerRecords(quiz, drafts, startedAt) {
...
function normalizeResponseForSubmission(question, response) {
	if (question.type === "numeric") {
		if (typeof response === "number" && Number.isFinite(response)) return response;
		if (typeof response === "string") return parseNumericResponse(response) ?? response.trim();
		return null;
	}
	return response;
}
function safeElapsedMs(draft, startedAt) {
	...
	const sessionStartedAt = startedAt ? Date.parse(startedAt) : NaN;
	const sessionElapsed = Number.isFinite(sessionStartedAt) ? Math.max(0, now - sessionStartedAt) : elapsed;
	return Math.min(elapsed, sessionElapsed, 7200 * 1e3);
}
function clampDraftFirstSeenAt(firstSeenAt, startedAt, now = Date.now()) {
	const sessionStartedAt = startedAt ? Date.parse(startedAt) : NaN;
	const floor = Number.isFinite(sessionStartedAt) ? sessionStartedAt : now - 7200 * 1e3;
	if (!Number.isFinite(firstSeenAt)) return now;
	return Math.max(Math.min(firstSeenAt, now), floor);
}
function getSubmitIssue(quiz, drafts, displayPolicy, activityPolicy, startedAt) {
	const completion = buildCompletionSummary(quiz, makeAnswerRecords(quiz, drafts, startedAt), displayPolicy, activityPolicy);
...
function formatSubmitIssue(quiz, missingQuestions, missingConfidence) {
	const parts = [];
...
function formatQuestionList(quiz, questionIds) {
...
function hasMeaningfulText(value) {
	return value.replace(/<\/?(?:u|sub|sup)>/gi, "").replace(/[*_`~]/g, "").trim().length > 0;
}
function isQuestionAnswerComplete(question, draft) {
	if (!draft) return false;
	const response = draft.response;
	const special = asSpecialResponse(response);
	if (special?.kind === "cancelled") return true;
	switch (question.type) {
		case "multiple_choice": return typeof response === "number" || special?.kind === "other" && special.text.trim().length > 0;
		case "multi_select": return Array.isArray(response) && response.length > 0 || special?.kind === "other" && special.text.trim().length > 0;
		case "true_false": return typeof response === "boolean";
		case "fill_blank":
		case "short_answer":
		case "long_response": return typeof response === "string" && hasMeaningfulText(response);
		case "numeric": return typeof response === "number" && Number.isFinite(response) || typeof response === "string" && hasMeaningfulText(response);
		case "matching": {
			const leftItems = getMatchingSide(question, "left");
			const rightIds = new Set(getMatchingSide(question, "right").map((item) => item.id));
		...
		case "text_select": return isTextSelectComplete(question, response);
		default: return hasResponse(draft);
	}
}
...
function isMatchingPairs(value) {
	return Array.isArray(value) && value.every((item) => item && typeof item === "object" && typeof item.leftId === "string" && typeof item.rightId === "string");
}
function normalizeConfidence(value) {
	return value === 1 || value === 2 || value === 3 ? value : void 0;
}
...
//#endregion
//#region src/main.tsx
function showFatalWidgetError(error) {
    ...
    <main class="shell narrow">
      <section class="card stack fatal-widget-error">
        <p class="eyebrow">BetterQuizzes V1</p>
        <h1>Widget error</h1>
...
var ErrorBoundary = class extends import_react.Component {
	...
	render() {
		if (this.state.error) return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("main", {
			className: "shell narrow",
			children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("section", {
				className: "card stack fatal-widget-error",
				children: [
					/* @__PURE__ */ (0, import_jsx_runtime.jsxs)("p", {
						className: "eyebrow",
						children: ["BetterQuizzes ", "V1"]
					}),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("h1", { children: "Widget error" }),
					/* @__PURE__ */ (0, import_jsx_runtime.jsx)("p", { children: "The widget hit a render error instead of going blank." }),
...
if (!rootElement) showFatalWidgetError(/* @__PURE__ */ new Error("Missing #root element."));
else {
	rootElement.dataset.bqMounted = "true";
	(0, import_client.createRoot)(rootElement).render(/* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_react.StrictMode, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(ErrorBoundary, { children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)(App, {}) }) }));
}
//#endregion
